...
const int LENGTH = 100036;
const float SCALE = 10;
const float ZOOM = 5;

__global__ void update(float* d_positions, float* d_old_positions){
    ...
    *p1x -= cos(angle) * (FORCE / distance);
    *p1y -= sin(angle) * (FORCE / distance);
    
    return;
}

__global__ void phys_update(float* d_positions, float* d_old_positions, Uint32* d_pixels){
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    float distance = fmaxf(sqrt(*p1x * *p1x + *p1y * *p1y), 10.0f);
    float angle = atan2(*p1y, *p1x);

    *p1x -= cos(angle) * (10 / distance);
    *p1y -= sin(angle) * (10 / distance);
...
int main(){
    ...
    while (running){
        ...
        update<<<dim3((LENGTH + threads.x - 1) / threads.x, (LENGTH + threads.x - 1) / threads.x), dim3(32, 32)>>>(d_positions, d_old_positions);
        
        phys_update<<<(LENGTH + threads.x - 1) / threads.x, 32>>>(d_positions, d_old_positions, d_pixels);
